...
:- pce_global(@name_prompter, make_name_prompter).
        ...
        % crea dialog para el ingreso del nombre
        ...
        %Muestra el dialogo para ingresar el nombre y buscar la informacion
        ...
        %Muestra infromacion de las plantas
        buscar_info_planta:-
...
%INTERFAZ
    run :-
            % Iniciar Programa
            new(D, dialog('Sistema Experto (El Yerberito ilustrado) | Josué Almanza Martinez')),
            send(D, size, size(340, 820)),

            % Mostrar la imagen
            send(D, append, new(Imagen, menu_bar)),
            mostrarImagen('C:/Sistema-Experto-Yerberito/imagenes/Inicio.jpg', D, Imagen),
            send(D, append, Imagen),
            ...
            send(P, append, button('Consultar plantas segun sus propiedades', message(@prolog, buscar_propiedad_plantas)), below),
            ...
            send(P, append, button('Listar plantas y sus propiedades', message(@prolog, metodo_listar_planta_efecto)),below),
            send(P, append, button('Listar elementos de las plantas', message(@prolog, metodo_listar_elementos)),below),
            send(P, append, button('Listar plantas y su metodo de preparacion', message(@prolog, metodo_listar_plantas_uso)), below),
            send(P, append, button('Listar propiedades y sus efectos', message(@prolog, metodo_listar_propiedades_efectos)), below),
            send(P, append, button('Listar plantas que producen medicamentos', message(@prolog, metodo_produce_medicamento)), below),
            send(P, append, button('Listar plantas', message(@prolog, metodo_listar_plantas)), below),
            send(P, append, button('Listar medicamentos', message(@prolog, metodo_lista_medicamentos)), below),
            send(P, append, button('Listar enfermedades', message(@prolog, metodo_lista_padecimientos)), below),
            send(P, gap, size(10, 10)), % Espacio entre elementos
            send(P, append, button('Nombres cientificos de plantas', message(@prolog, metodo_listar_planta_y_nombre_cientifico)), below),
            send(P, append, button('Origenes de las plantas', message(@prolog, metodo_lista_origenes)), below),
            %send(P, append, button('Definicion de terminos', message(@prolog, buscar_Termino)), below),
            send(P, append, button('Botiquin de emergencias', message(@prolog, metodo_botiquin)), below),
            % Agregar el panel a la derecha.
            send(D, append, P, next_row),
            

            % Abrir el diálogo
            send(D, open, point(250, 130)),
...
%INFORMACION DE PLANTA
    metodo_info_planta(Planta):-
        ...
        send(D, display, text('Cura', left, bold), point(200,170)), nl,
        send(D, display, text(EnfermedadesStr, left, normal), point(210,185)),

        send(D, display, text('Produce Medicamentos', left, bold), point(10,170)),nl,
        send(D, display, text(MedicamentosStr, left, normal), point(20,185)),

        send(D, display, text('Propiedades', left, bold), point(10,230)), nl,
        send(D, display, text(EfectosStr, left, normal), point(20,245)),

        send(D, display, text('Metodos De Preparacion', left, bold), point(10,300)),nl,
        send(D, display, text(FormasUsoStr, left, normal), point(20,315)),    
...
concatenar_s(Propiedad, PropiedadConS) :-
    atom_concat(Propiedad, 's', PropiedadConS).
...
metodo_listar_plantas_propiedad(Propiedad):-
    ...
    concatenar_s(Propiedad, PropiedadConS),
    atom_concat('Plantas que son ', PropiedadConS, Titulo),
    new(D, dialog(Titulo)),
    send(D, size, size(400,500)),
    send(D, colour, colour(black)),

    send(D, display, text(Titulo, left, bold), point(10,10)),
    new(W, window("Plantas con la propiedad: ", size(370, 460))),
    send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
    send(W, display, text(PlantasStr, left, normal), point(5, 5)),
...
%INFORMACION DE MEDICAMENTO
    metodo_info_medicamento(Medicamento):-
        ...
        atom_concat('', Medicamento, Titulo),
        new(D, dialog(Titulo)),
        send(D, size, size(400,500)),
        send(D, colour, colour(black)),
...
%INFORMACION DE ENFERMEDAD
    metodo_info_enfermedad(Enfermedad):-
        ...
        send(D, append, text('Planta y metodo de preparacion que la cura', left, bold)),
        new(W, window('Propiedades de plantas y usos', size(380, 420))),
        send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
    ...
    % Mostrar los resultados en la ventana aplicando recursividad.
        ...
        mostrar_resultados2([[Planta, FormaUso] | Resto], W, Y) :-
            ...
            send(W, display, text(FormaUso, left, normal), point(230, Y)),
...
metodo_enfermedades_curadas_por(Planta):-
    %Poner nombre a la
    atom_concat('Enfermedades curadas usando ', Planta, Titulo),
    new(D, dialog(Titulo)),
    ...
    %Buscar si una planta esta ligada con un malestar
    findall(Malestar, usado_para_tratar(Planta, Malestar), Enfermedades),
    send(D, open, point(500, 200)),

    %Mostrar planta seguido del nombre
    send(D, display, text('Planta ', left, bold), point(200,15)),
    send(D, display, text(Planta, left, normal), point(210,35)),
    ...
    send(D, display, text(EnfermedadesStr, left, normal), point(210,80)),
...
metodo_forma_preparacion(FormaUso):-
    ...
    send(D, display, text('Ingredientes', left, bold), point(10,20)),
    send(D, display, text(Ingredientes, left, normal), point(15,40)),

    send(D, display, text('Se necesita', left, bold), point(110,20)),
    send(D, display, text(Partes, left, normal), point(115,40)),
    
    send(D, display, text('Tiempo', left, bold), point(250,20)),
    send(D, display, text(Tiempo, left, normal), point(255,40)),
    
    send(D, display, text('Via De Administracion', left, bold), point(10,90)),
    send(D, display, text(Administracion, left, normal), point(10,110)),

    send(D, display, text('Instrucciones', left, bold), point(10,140)),
    send(D, display, text(Instrucciones, left, normal), point(15,160)).


%LISTA DE PLANTAS Y ELEMENTOS    
    metodo_listar_elementos:-
        new(D, dialog('Listar Elementos De Las Plantas')),
        send(D, size, size(310, 435)),
        send(D, colour, colour(black)),
        findall(Medicamento, medicamento(Medicamento), Medicamento),
        %Crear una ventana con scroll
        send(D, display, text('Elementos que se encuentran en las plantas', center, bold), point(0, 1)),
        new(W, window('Elementos que se encuentran en las plantas', size(250, 405))),
        send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
        ...
        send(W, display, text(MedicamentoStr, left, normal), point(5, 5)),
        %Agregar la ventana con scroll al diálogo
        send(D, append, W),
        %Abrir el diálogo
        send(D, open, point(800, 200)).

%LISTA PLANTAS Y METODO DE PREPARACION
metodo_listar_plantas_uso :-
    new(D, dialog('Listar plantas y su metodo de preparacion')),
    send(D, size, size(400, 450)),
    send(D, colour, colour(black)),
    ...
    % Crear una ventana con scroll
    % Crear una ventana con scroll
    send(D, append, new(text('Planta                           Metodo de preparacion', center, bold))),
    new(W, window('Planta                           Metodo de preparacion', size(370, 400))),
    send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
...
%LISTA PLANTAS QUE PRODUCEN MEDICAMENTOS
...
metodo_lista_medicamentos:-
    ...
    %Mostrar los resultados de la lista en la ventana con scroll
    send(W, display, text(ListaMedicamentos, left, normal), point(10, 10)),
...
metodo_lista_padecimientos:-
    ...
    %Mostrar los resultados de la lista en la ventana con scroll
    send(W, display, text(ListaEnfermedades, left, normal), point(10, 10)),
...
metodo_listar_propiedades_efectos :-
    ...
    % Crear una ventana con scroll
    send(D, append, new(text('Propiedad                     Efecto', center, bold))),
    new(W, window('Propiedad                     Efecto', size(430, 360))),
    send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
...
metodo_listar_planta_efecto:-
    ...
    % Crear una ventana con scroll
    send(D, append, new(text('Planta                                               Propiedad', center, bold))),
    new(W, window('Planta                                               Propiedad', size(400, 370))),
    send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
...
metodo_listar_planta_y_nombre_cientifico:-
    ...
    new(W, window('Nombre Comun                                Nombre cientifico', size(480, 420))),
    send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
    
    % Mostrar los resultados en la ventana con scroll
    mostar_resultado_planta_nombreCientifico(Resultados, W, 0),
...
metodo_lista_origenes:-
    new(D, dialog('Nombre Cientifico De Plantas')), 
    send(D, size, size(480, 450)),
    send(D, colour, colour(black)),
    
    % Buscar todas las plantas y sus orígenes
    findall([Planta, Origen], planta_origen(Planta, Origen), Resultados),
    
    % Crear una ventana con scroll
    send(D, append, new(text('Nombre Comun                                Origen', center, bold))),
    new(W, window('Nombre Comun                                Origen', size(460, 420))),
    send(W, scrollbars, vertical),  % Agregar barra de scroll vertical

    % Mostrar los resultados en la ventana con scroll
    mostrar_resultado_planta_origen(Resultados, W, 0),
    ...
    mostrar_resultado_planta_origen([[Planta, Origen] | Resto], W, Y) :-
        send(W, display, text(Planta, left, normal), point(10, Y)),
        send(W, display, text(Origen, left, normal), point(230, Y)),
        NewY is Y + 20,  % Incrementar la posición Y para la siguiente línea
...
%BOTIQUIN  
    metodo_botiquin :-
        ...
        send(D, size, size(270, 380)),
        send(D, colour, colour(black)),
        findall(Planta, botiquin(Planta), Plantas),
        
        %Crear una ventana con scroll
        send(D, display, text('Plantas escensiales para un botiquin', left, bold), point(10, 10)),
        new(W, window('Plantas escensiales para un botiquin', size(250, 350))),
        send(W, scrollbars, vertical),  % Agregar barra de scroll vertical
